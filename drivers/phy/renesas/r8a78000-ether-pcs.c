...
static int r8a78000_eth_pcs_init(struct phy *p)
{
	...
	if (ret) {
		pr_debug("XPCS: Failed to init mphy\n");
		return ret;
	}
...
static int r8a78000_eth_pcs_probe(struct udevice *dev)
{
	...
	if (!dd->addr)
        ...
        /* TODO: Enable reset control when supported by hardware/DT
         *
         * Currently commented out because reset line is not yet
         * available or not described in the device tree.
         *
         * Once available, enable the following:
         */
